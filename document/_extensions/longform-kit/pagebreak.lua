...
local function newpage(format, pagebreak)
  ...
  elseif format:match 'gfm' or format:match 'markdown' then
    return pandoc.RawBlock('html', pagebreak.html)
...
--- Converts the portable Longform Kit pagebreak Div to the target format.
local function div_pagebreak(raw_pagebreak)
  return function (div)
    if not div.classes:includes('pagebreak') then
      return nil
    end
    if #div.content > 0 then
      error('Longform Kit: a pagebreak Div must be empty')
    end
...
function Pandoc (doc)
  ...
  return doc:walk {
    Div = div_pagebreak(raw_pagebreak),
